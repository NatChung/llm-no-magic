...
- 點深看:五顆泡泡、五個展開器,展開的都是它自己那則訊息。灰色 user 泡
  ...
  `role: "tool"`,內容是**完整的 SKILL.md 全文**,躺在那給學生看;藍色 turn 2
  ▸ 模型的原始 `run_script` 呼叫;紫泡是「展開器=某份 prompt」這條規則唯一刻意的例外:
  它展開的是「腳本原始碼」— 那份 code 從頭到尾沒進過任何一份 model 看過的 prompt;
  綠泡 ▸「送給使用者的原始訊息」— 模型最終的回覆。帶學生自己展開琥珀泡看注入現場就好,
  不用另外找什麼提示 — 那顆泡泡自己的展開器**就是**那發 sent prompt。
...
- 「context 改機率」的三個互扣證據,一次收線:
  ...
  2. **無 skill 對照**(行為):同 model 同一句,索引在/不在 → 「28°C 晴」變
     「我無法提供即時資訊」
  3. **琥珀泡自己的展開器**(物證):它的「送給 AI 的 prompt(turn 2)」— SKILL.md
