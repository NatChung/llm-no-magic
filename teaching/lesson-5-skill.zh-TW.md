...
- 點深看:六顆泡泡、七個展開器 —— 除了紫色腳本泡,每個泡泡展開的都是它自己那則訊息;紫泡有兩個,同時展「腳本原始碼」和「把腳本結果餵回模型的那份 prompt」。灰色 user 泡
  ▸「送給 AI 的 prompt(turn 1)」— 跟 tab④/⑥ 同一套 `<|im_start|>` 累積視圖:system + user,
  還沒有半點 skill 的影子;藍色 turn 1 ▸「模型吐的原始訊息」— 模型的 `read_file` 呼叫;
  琥珀泡 ▸「送給 AI 的 prompt(turn 2)」— 這**就是**注入的現場:累積視圖裡多了一則
  `<|im_start|>user` 區塊,內容是 `<tool_response>` 包住**完整的 SKILL.md 全文**
  (字元數很大,約 691 字元)。展開泡泡一次,SKILL.md 就躺在那給學生看 — 不用再折
  第二層。這一則同時也被琥珀底標起來、尾巴掛「← 這次新增、要送出的」,因為它是
  結尾空 assistant 生成提示之前最後一則非空訊息 — highlight 直接點名了注入現場;
  藍色 turn 2 ▸ 模型的原始 `run_script` 呼叫;紫色腳本泡有**兩個**展開器,是很漂亮的對比
  ▸「腳本原始碼」— 那份 code 從頭到尾**沒進過任何一份 model 看過的 prompt**,以及
  ▸「送給 AI 的 prompt(turn 3)」— 就是把工具結果 `<tool_response>` 包起來餵回模型、換出最終
  答案的那份 prompt(琥珀 highlight 標在那則餵回的工具結果上)。兩個並排、就在這顆「結果餵回
  模型」的泡泡下,學生一眼看懂「model 只吃到腳本的**輸出**、從沒看過它的**原始碼**」;綠泡
  ▸「送給使用者的原始訊息」— 模型最終的回覆。帶學生:這是一次點擊、不是
  兩次 — 展開琥珀泡,SKILL.md ——已經被琥珀 highlight 標成這一發的新輸入——就在那。
...
- 「context 改機率」的三個互扣證據,一次收線:
  ...
  2. **無 skill 對照**(行為):同 model 同一句,索引在/不在 → 「28°C 晴」變
     「我無法提供即時資訊」
  3. **琥珀泡自己的展開器**(物證):展開泡泡看到「送給 AI 的 prompt(turn 2)」—
     SKILL.md 就是裡面那則 `<|im_start|>user` 區塊,琥珀底、尾巴標
     「← 這次新增、要送出的」— 注入現場,UI 自己都標出來了
