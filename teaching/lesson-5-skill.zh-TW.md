...
- 點深看:五顆泡泡、五個展開器,展開的都是它自己那則訊息。灰色 user 泡
  ▸「送給 AI 的 prompt(turn 1)」— `messages[]` 只有兩則,system + user,還沒有半點
  skill 的影子;藍色 turn 1 ▸「模型吐的原始訊息」— 模型的 `read_file` 呼叫;琥珀泡
  ▸「送給 AI 的 prompt(turn 2)」— 這**就是**注入的現場:`messages[]` 裡多了一則
  `role: "tool"`,內容是**完整的 SKILL.md 全文**。這則太長,JSON 樹預設把它折起來,
  標成類似 `▸ {…} 2 個欄位,428 字元` 的摘要 — 點開它,SKILL.md 就躺在那給學生看;
  藍色 turn 2 ▸ 模型的原始 `run_script` 呼叫;紫泡是「展開器=某份 prompt」這條規則唯一
  刻意的例外:它展開的是「腳本原始碼」— 那份 code 從頭到尾沒進過任何一份 model 看過的
  prompt;綠泡 ▸「送給使用者的原始訊息」— 模型最終的回覆。帶學生:這是兩次點擊、不是
  一次 — 先展開琥珀泡看到 turn 2 的 prompt,再點開裡面折疊的 `role: "tool"` 節點 —
  SKILL.md 就藏在那第二層折疊裡。
...
- 「context 改機率」的三個互扣證據,一次收線:
  ...
  2. **無 skill 對照**(行為):同 model 同一句,索引在/不在 → 「28°C 晴」變
     「我無法提供即時資訊」
  3. **琥珀泡自己的展開器,再折一層**(物證):先展開泡泡看到「送給 AI 的
     prompt(turn 2)」,再展開裡面折疊的 `role: "tool"` 節點 — SKILL.md
     就躺在 messages 裡 — 注入的現場
