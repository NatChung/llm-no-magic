...
- **收尾接龍(帶出下一課)**:開著 thinking 換一題 —
  `POST /drive {"tab":"3","user":"現在幾點?","mode":"thinking"}`。盯著 thinking 區看:model
  會認真推理,但它會在裡面自己講「我這裡沒有實體時鐘…我要怎麼知道現在幾點?」,最後只能**憑空
  假設 / 編一個時間**。跟學員說:**連 thinking 都問不出真實時間 —— 它需要一個工具**。記住這句
  「現在幾點?」,下一課同一句會自動帶到下一個分頁,那裡它會真的去拿時間 → Lesson 4
