...
v3 帶課由 AI 打 `POST /drive` 驅動、透過 `/events` 斷言頁面反映(見 AGENTS.md / lesson playbook);
這些 Python script 降為 creator 跑 `--smoke` 的頁面回歸驗證(確認 preset/selector/生成流程沒漂移)。學生不用 pip playwright。

Shared Playwright helpers for the demo smoke harness.

契約(對應 frontend/app.js):
- `POST /drive` → 頁面自動切到被驅動的 tab(`main.tab-panel.active[data-panel=…]` 出現)+ 渲染 token/turn
- model swap 中 body.swapping;swap 失敗 → alert() → 我們的 dialog handler 接住(這支還是需要,不能拔)
