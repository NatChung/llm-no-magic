# Lesson 4 — Tab ④ 代理:tool_call 約定與真執行(+ 整課收尾)
...
- **你(AI)可以直接代點展開器**:`POST /inspect {"action":"expand","tab":"4","role":"sent","turn":1}`
  (收合用 `"action":"collapse"`;role:`sent`=送給 AI 的 prompt、`raw`=模型原始訊息、
  `final`=給使用者的原始訊息,`turn` 選填、省略取最後一顆)。頁面會自動切到該分頁並捲到該
  展開器。**帶法:一顆一顆來** — 展開 → 講解 → 等學員說懂了 → 收合 → 下一顆,不要一次全開
- 想看細節:先講清楚展開器裡是什麼 — 每個 ▸ 展開器打開的是一份上色、可折疊的
  「wire view」原始文字;灰色是 `<|im_start|>`/`<|im_end|>` 這類 template 標記,箭頭就是
  負責折疊/展開每個區塊的開關。每顆泡泡下面都**剛好一個** ▸ 展開,而且展開的永遠是「它
  自己那則訊息」—— 右側泡泡秀的是因它而送出的 prompt;左側藍/綠泡秀的是模型自己吐出來的
  東西。帶學員四顆都展開一輪:灰色 user 泡 ▸「送給 AI 的 prompt(turn 1)」—
  chat-template 過的字串;展開後訊息是一排收合的清單(你的問題那則、被標成「這次新增」的,
  預設展開)。點開 `<|im_start|> system` 那則,裡面就是 `<tools>` 區塊 —— `get_time` 躺在
  裡面,**這時還沒有工具回傳結果**;藍泡
  ...
  接龍累積的證據。這些 prompt 裡最後一則(琥珀底、標「← 這次新增、要送出的」)都是
  這一發剛加進來、要送給 model 的新輸入 —— turn 1 是你的問題,之後是餵回的工具結果。
