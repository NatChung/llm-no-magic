...
Verified 2026-05-29. **Two-port setup** — one Python server on `:9000` serves HTML + API + SSE; `llama-server` on `:8080` runs the model (0.6B or 4B, swapped automatically inside `/drive`). Backend `:8082` from earlier architectures has been merged into `:9000`.

| Port | Purpose | When it runs |
|------|---------|--------------|
| **:8080** | llama-server (0.6B or 4B, swapped inside `/drive`) | auto-managed |
...
| :8080 | llama-server (0.6B or 4B, swapped inside `/drive`) | always (controlled by `/drive`) |

Full Tab ④ flow: browser page → `POST /drive {tab:4}` → server runs the agent loop, swaps to 4B inside `/drive` if needed → `:8080` llama-server; the page renders via its `/events` subscription.
...
- **Fall back to CLI `agent.py`**: first confirm 4B is on `:8080` (`curl -s http://localhost:8080/v1/models | grep -q "Qwen3-4B"`); if not, launch 4B manually (see the next bullet: pkill + nohup llama-server with the 4B GGUF). Then run `python3 -m agent.agent`.
