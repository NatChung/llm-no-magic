...
- **行號皆為「改檔前」原始行號** — 同一 task 內先刪的步驟會讓後續行號位移，執行時一律以引用的內容字串定位
...
順手 grep 該檔 Tab ⑧ 內文有無「§0」引用（flip-table 原本是 §0；若內文提及就一併改寫或刪）。
...
Run: `grep -c "hook" frontend/index.zh-TW.html; grep -c "v=56" frontend/index.zh-TW.html`
Expected: `0` 與 `2`（注意：count 為 0 時 grep exit code = 1，屬正常，不是失敗）
...
行號與 zh-TW 版相同（Hook A 98-137 / content 138-139 + 164；Hook B 340-372 / content 373-374 + 443；recap 679 + 檢查「§0」引用；script tag 858）。stale comment 修成：`Tab 4: Agent — real tool execution (server :9000 /agent)`。所有 `?v=55` → `?v=56`。

- [ ] **Step 3: 驗證**

Run: `grep -c "hook" frontend/index.html; grep -c "8082" frontend/index.html frontend/index.zh-TW.html`
Expected: 全部 `0`（grep exit 1 屬正常）
...
git commit -m "refactor(web): drop hooks.js + hook CSS"
```
...
Run: `for n in 1 2 3 4; do python3 teaching/demos/demo_tab$n.py --smoke || { echo "FAIL tab$n"; break; }; done`
Expected: 4 個 `DONE`
